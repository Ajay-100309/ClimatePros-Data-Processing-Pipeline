...
venv/bin/python process.py --chunk-size 1000    # A->B->D per 1000 (default 500; 0 = whole batch at once)
...
`state/` and `output/` are **irreplaceable live data, not build artifacts** — the small, identity-critical files are committed to git, but the bulk is deliberately untracked (GitHub's 100 MB blob limit): `state/embeddings.npy` + `embeddings_index.json` (content-addressed embedding cache — rebuildable via the gateway, `stage_index` re-embeds on demand), `state/dispatch_meta.json` (display metadata — re-queryable only from the DB), `state/notes_class.json` (Stage A classifications — grows ~16.6 MB per 10k dispatches, the fastest-growing file by far; historical entries are **never read again** because `stage_notes.run` only consults it to skip dispatches in the *current* batch and the ledger stops those being refetched, so only `make_report.py`'s aggregate stats and the rare `incomplete_stage_a` retry depend on it), and `state/batches/` + `batch_current.json` (audit archives / transient work order). Move the untracked bulk between machines with `./sync_state.sh pull|push user@host [remote_repo_path]`. Together they hold the append-only ledger, the case registry, the embedding cache, the recorded-parts snapshot, the search push record, and the Chroma vector DB of every case. Check `pipeline.py --stats` for current totals (16,000 ledger dispatches / 1,808 cases / 12,744 indexed as of 2026-08-28).
...
- **The work order crosses machines through git.** `state/batch_current.json.gz` is gzipped (~7 MB per 5,000 dispatches, 3x) and **tracked**, because the fetch half needs VPN to FieldJetXStg while the process half does not — so the usual split is fetch on a laptop, commit, pull on the server, `process.py` there. `statefiles.load_json`/`save_json` gzip transparently on a `.gz` suffix (the temp file keeps the suffix, or it would be written uncompressed and renamed); `stage_fetch.load_batch()` still falls back to the pre-gzip `batch_current.json`. `process()` calls `stage_fetch.merge_dispatch_meta(light=True)` first, rebuilding the display fields Stage D needs (`reason`, `dispatch_number`, `received_dt`) from the work order itself — `dispatch_meta.json` is 100 MB+ and untracked, so it never travels. Light records omit `combined_notes` (~99% of each record, and read only by `reports.py` for the legacy `--with-cases` spreadsheet, via `.get` with a default), so a process-only machine's copy stays ~20 MB at 160k dispatches instead of ~336 MB. Deleting that machine's `dispatch_meta.json` is therefore safe and self-healing — it rebuilds per batch. `finalize` deletes the work order, and that deletion reaches the other machine on its next pull.
- **Stale work order**: with an rsync handoff (no git deletion), a batch processed elsewhere leaves a stale copy that would block every future fetch. `stage_fetch._clear_if_already_processed` removes it when `state/batches/<batch_id>.json` exists (sync the archives first with `./sync_state.sh pull`). `--dry-run` ignores the guard entirely, since it writes nothing.
...
`pipelib/runner.py` orchestrates the `pipelib` stages over a staged batch, then finalizes; the entry points only parse arguments. `process()` works through the batch in **slices** of `config.PROCESS_CHUNK = 500` (`--chunk-size`, 0 disables), running A → B → (C) → D per slice so documents reach the search index throughout a long run instead of only after the last dispatch is extracted — a 10k batch otherwise pushes nothing for ~7h. Results are identical either way: every stage loads the *global* state file, skips what it has already done, and returns that global state, so `finalize()` still sees everything. Slicing costs ~5s per 10k batch (extra `load_json` calls) and preserves Stage C's strict sequencing, since slices run in order. All state writes go through `statefiles.save_json` (tmp + `os.replace`, atomic) and every stage checkpoints every `CHECKPOINT_EVERY = 10` items, so the process can be killed at any point and resumed by re-running the same command. `runner.check_state(with_cases)` validates the ledger up front in both halves; it additionally validates the casemap fingerprint **only when Stage C will run** — a fingerprint mismatch must never surface after a batch of LLM calls has been paid for, and `runner.process()` likewise calls `config.require_search_config()` before any LLM spend in the default mode.
