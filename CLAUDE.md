...
- **The work order crosses machines through git.** `state/batch_current.json.gz` is gzipped (~7 MB per 5,000 dispatches, 3x) and **tracked**, because the fetch half needs VPN to FieldJetXStg while the process half does not — so the usual split is fetch on a laptop, commit, pull on the server, `process.py` there. `statefiles.load_json`/`save_json` gzip transparently on a `.gz` suffix (the temp file keeps the suffix, or it would be written uncompressed and renamed); `stage_fetch.load_batch()` still falls back to the pre-gzip `batch_current.json`. `process()` calls `stage_fetch.merge_dispatch_meta()` first, rebuilding the display fields Stage D needs (`reason`, `dispatch_number`, `received_dt`) from the work order itself — `dispatch_meta.json` is 100 MB+ and untracked, so it never travels. `finalize` deletes the work order, and that deletion reaches the other machine on its next pull.
